...
def ensure_data(db: Session) -> None:
    ...
    plan = [
        # b0 СПб (3)
        ("ООО НордСофт", 0, ["web"], 4),
        ("ООО ПитерКофе", 0, ["coffee"], 2),
        ("ООО Питер-Электро", 0, ["elec"], 5),
        # b1 Москва (4)
        ("ООО МосТех Аналитика", 1, ["bi", "ds"], 2),
        ("АО СтолицаФарм", 1, ["pharmacy"], 1),
        ("ООО Москва-Ресторан", 1, ["rest"], 3),
        ("ООО Москва-Одежда", 1, ["cloth"], 2),
        # b2 Казань (3)
        ("ИП Бауман Дев", 2, ["mobile", "web"], 1),
        ("ОАО КазаньМолоко", 2, ["milk"], 2),
        ("ООО Казань-Кофе", 2, ["coffee"], 1),
        # b3 Новосибирск (2)
        ("АО СибСтом", 3, ["dental"], 1),
        ("ООО Новосиб-Онлайн", 3, ["online"], 1),
        # b4 Екатеринбург (2)
        ("ИП Петров DevOps-аутсорс", 4, ["devops", "sre"], 3),
        ...
        # b7 Уфа (2)
        ("ИП УфаКлининг", 7, ["clean"], 2),
        ("ООО УфаКофе", 7, ["coffee"], 1),
        # b8 Пермь (1)
        ("ООО ПермьТехСервис", 8, ["devops"], 3),
        # b9 Ростов (3)
        ("ООО РостовРесторан", 9, ["rest"], 1),
        ("ООО РостовОдежда", 9, ["cloth"], 4),
        ("ООО Ростов-Логистика", 9, ["logi"], 2),
        # b10 Воронеж (2)
        ...
        # b15 Владивосток (2)
        ("ООО ВладивостокТех", 15, ["web", "devops"], 4),
        ("ООО ВладКофе", 15, ["coffee"], 1),
        # b16 Омск (1)
        ("ООО ОмскОдежда", 16, ["cloth"], 2),
        # b17 Томск (2)
        ("ООО ТомскУнивер", 17, ["univ"], 1),
        ("ООО ТомскОнлайн", 17, ["online"], 2),
        # b18 Тула (1)
        ("ООО ТулаЛогистика", 18, ["logi"], 2),
        # b19 Хельсинки (3)
        ("FinEd Tutor Oy", 19, ["langs", "online"], 1),
        ("Helsinki Coffee Roasters", 19, ["coffee"], 2),
        ("Helsinki Analytics Oy", 19, ["bi"], 2),
        # b20 Таллин (3)
        ("Tallinn Tech OÜ", 20, ["web", "sre"], 3),
        ("Tallinn Dental OÜ", 20, ["dental"], 2),
        ("Tallinn Market OÜ", 20, ["mkt"], 5),
    ]
